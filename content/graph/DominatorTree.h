 ...
 * Status: Tested https://judge.yosupo.jp/submission/310812
 */
...
int main() {
	int city_num, flight_num; std::cin >> city_num >> flight_num;
	vector<vector<int>> adj(city_num), rev_adj(city_num);
	for (int f = 0; f < flight_num; f++) {
		int a, b; std::cin >> a >> b;
		adj[--a].push_back(--b); rev_adj[b].push_back(a);
	}
	vector<int> visit_order, visit_time(city_num, INT32_MAX), visit_parent(city_num, -1);
	vector<bool> visited(city_num, false);
	std::function<void(int, int)> dfs = [&](int at, int prev) {
	...
	dfs(0, -1);
	vector<int> dsu_parent(city_num, -1), dsu_min(city_num, INT32_MAX);
	...
	vector<int> idom(city_num, -1); // idom array is the result ominator tree.
    // idom[x] is its immediate dominator (parent in ancestor tree).
	for (int c : visit_order) {
